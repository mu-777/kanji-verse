...
## 追記（2026-06-25）: エンコーダを WebCodecs に刷新（ADR-0011）
実機(Windows)検証で MediaRecorder 版に2つの問題が判明し、方式を変更した。
- **Windows プレイヤーで終端エラー＋1回しか再生不可** → ffprobe で原因特定: **A/V長不一致**（音声が映像より約0.33s長い）。fix1（音声トリム）で解消をユーザー確認。加えて単一キーフレーム/VFR。
- **実測 ≒12fps のカクつき** → 1080p H.264 のリアルタイムエンコードが追いつかない。
- 対応: **WebCodecs + mp4-muxer のオフライン生成**へ全面切替（CFR30/1秒キーフレーム/A-V同尺、Android MP4 確定）。音声は録画専用 `public/audio/share-clip.mp3` を AAC 合成（ユーザー合意）。`audio.ts` の録画キャプチャAPIは撤去。WebCodecs 非対応端末は Share 非表示。尺は 10秒＋終端2秒フェードに変更。詳細は ADR-0011。
- [ ] **次の検証**: 生成→DL したファイルを ffprobe で CFR/キーフレーム間隔/A-V同尺を客観確認。
