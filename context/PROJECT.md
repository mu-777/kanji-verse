...
## 最終更新
2026-07-01（welcome 表示中の BGM 早期再生＝最初のジェスチャで開始。それ以前: 検索タイトル書き換え対策: h1=title 一致 ／ ソーシャル動画シェア, ADR-0010+0011 ／ インタラクティブBGM, ADR-0009 ／ SEO, ADR-0008）
...
- **welcome 表示中の早期再生（2026-07-01）**: sound=on で welcome を出している間、最初のユーザージェスチャ（pointerdown/keydown/touchstart）で BGM を鳴らし始める（`main.ts` の welcome ブロック）。従来は pref=on の再訪時に `setEnabled` が未呼び出しのまま Explore のクリックまで無音だった。オートプレイ規制でジェスチャ前には鳴らせないため「完全に無操作で鳴る」ことは不可。welcome 上の最初の操作で開始する＝「Explore を押す前から」鳴る。OFF→ON トグル経路は元々その click で再生される。
