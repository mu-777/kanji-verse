...
## 最終更新
2026-06-25（ソーシャル動画シェア, ADR-0010 ＋ WebCodecs 刷新 ADR-0011 ／ インタラクティブBGM, ADR-0009 ／ SEO, ADR-0008）
...
## 現在の課題・未解決事項
- **ソーシャル動画シェア: 実機検証が未実施（ADR-0010/0011）**。型チェック・ビルド通過済み。Windows終端エラーの主因は A/V長不一致で確定（WebCodecs で同尺化して解消見込み）、12fpsカクつきは WebCodecs オフライン(CFR30)で解消見込み。残り: ① 生成→DL したファイルを ffprobe で CFR/キーフレーム間隔/A-V同尺を客観確認 ② iOS/Android 実機で「漢字選択→Share→生成→プレビュー→Share→共有シートにX/LINEで動画添付（DL不要）→フィードで1:1動画再生」 ③ なめらかさ・終端フェードの見た目確認。IG は best-effort。録画用 `public/audio/share-clip.mp3` は仮（bgm 抜粋）で要差し替え検討。
