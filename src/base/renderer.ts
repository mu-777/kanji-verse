...
const LABEL_SCALE_START = 0.1;
const LABEL_SCALE_END = 0.6;
...
export class KanjiRenderer {
  ...
  search(kanji: string) {
    if (!kanji) { this.searchNode = null; this.requestRender(); return false; }
  ...
  private render() {
    ...
    const fontScreenSize  = Math.min(152 * Math.sqrt(scale), 468);
    const specialFontScreenSize = Math.min(172 * Math.sqrt(scale), 624);
    ...
    for (const node of this.nodes) {
      ...
      if (isSearch || isSelected || isHovered) {
    ...
    for (const node of specialNodes) {
      ...
      const ny = node.y * WORLD_SIZE + PADDING;
      const isSearch = node === this.searchNode;
      const color = isSearch ? COLOR_SEARCH : "#ffffff";
      ctx.globalAlpha = 1;
      ctx.fillStyle = color;
  ...
  private onMouseMove = (e: MouseEvent) => {
    ...
    if (prev !== this.hoveredNode) this.requestRender();
  };
